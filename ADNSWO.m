...
% 在ISWO上，删掉解的范围中心对称，删掉种群范围对称，改为开发随机选一个和种群最优求均值。


% The Spider Wasp Optimizer
function [Best_score,Best_SW,Convergence_curve]=SWO7(SearchAgents_no,Tmax,ub,lb,dim,fobj)
    FES = 0; %设定评估次数
 %%%%-------------------Definitions--------------------------%%
 %%
    ...
    if FES >= dim * 10000
     ...
     if rand<TR %% 3.2	Hunting and nesting behavior
        ...
        for i=1:SearchAgents_no
           ...
           r4=rand(); % r3 is a random number in [0,1]
           r5=rand(); % r3 is a random number in [0,1]
           ...
           for j=1:size(Positions,2)
             if i<k*SearchAgents_no
                if p < 2 * rand * (1-t/Tmax) ^ 2 %% 3.2.1	Searching stage (Exploration)
                   
                   Pa = k * SearchAgents_no / 2; % 种群被分成2份，Pa为1号种群个体数，SearchAgents_no - Pa为2号种群个体数
 ...
 %                       Positions(i,j)=Positions(JK(i),j)+m2*(lb(j)+rand*(ub(j)-lb(j))); %% Eq. (6)
                 ...
                 if r1<r2
                     Positions(i,j)=Best_SW(j)+cos(2*l*pi)*(Best_SW(j)-Positions(i,j));      % Eq. (16)
                         ...
                         %  if r4 < r5
                        %  -----------------------------------旋转捕猎---------------------------------------------
                          number_down = ceil(k*SearchAgents_no);
                          number_up = SearchAgents_no;
                          random_integer = randi([number_down, number_up]);
                          midpoint = ( Positions(random_integer, j) + Best_SW(j) ) / 2;
                          symmetric_point = 2 * midpoint - Positions(i, j);   %新的对称点
                          


                                                   if  symmetric_point  >   ub(j)
                                                      symmetric_point  =lb(j) + rand*(ub(j)-lb(j));
                                                   elseif symmetric_point < lb(j)
                                                       symmetric_point  =lb(j) + rand*(ub(j)-lb(j));
                                                   end        
%                                                    范围检测
                                                    ...
                                                    % 计算每个变量与给定值d之间的距离
                                                    dist_a = abs(Positions(i, j) - Best_SW(j) );
                                                    dist_b = abs(symmetric_point   -     Best_SW(j) );
 
                                                    if dist_b < dist_a
                                                        Positions(i, j) = symmetric_point;
                                                    end
          ...
          FES = FES + 1;%% The fitness value of the newly generated spider
     ...
     SearchAgents_no_old = SearchAgents_no;
     SearchAgents_no=fix(N_min+(SearchAgents_no-N_min)*((Tmax-t)/Tmax)); %% Eq. (25)
     if SearchAgents_no_old > SearchAgents_no 
       Positions(SearchAgents_no, :) = mean(Positions(SearchAgents_no: SearchAgents_no_old, :)); 
     end
